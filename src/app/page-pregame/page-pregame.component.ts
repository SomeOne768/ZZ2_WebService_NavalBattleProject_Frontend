...
export class PagePregameComponent implements AfterViewInit {
  ...
  constructor(private service: PagePregameService) {
    ...
    this.service.getAssociatedShip().subscribe(r => {
      this.associatedShips = r;
      this.associatedShips.forEach((ship, index) => {
        ship.located = 0;
        ship.position = [-1, -1];
        ship.id = index;
        ship.orientation = 0;
      });
      console.log(this.associatedShips);
    })
    ...
    if (this.dragging) {
      ...
      this.ships.forEach((child) => {
        ...
        if (ship_id == child.nativeElement.id) {
          ...
          this.associatedShips[ship_id].position[0] = child.nativeElement.getBoundingClientRect().left;
          this.associatedShips[ship_id].position[1] = child.nativeElement.getBoundingClientRect().top;
        }
      });
      // on regarde si le coin en haut a gauche du bateau est dans la map
      if (
        this.associatedShips[ship_id].position[0] > this.gridDisplayPosition.x
        && this.associatedShips[ship_id].position[0] < this.gridDisplayPosition.x + this.gridDisplayPosition.w
        && this.associatedShips[ship_id].position[1] > this.gridDisplayPosition.y
        && this.associatedShips[ship_id].position[1] < this.gridDisplayPosition.y + this.gridDisplayPosition.h
      ) {
        ...
        countOnX = Math.ceil((this.associatedShips[ship_id].position[0] - this.gridDisplayPosition.x) / 56) - 1; // 52 = largeur case (50px) + border de chaque cote (1px par cote) + ecart entre 2 cases (2.44px)
        countOnY = Math.ceil((this.associatedShips[ship_id].position[1] - this.gridDisplayPosition.y) / 56) - 1;
        //mettre le bateau dans la grille
        // horizontale
        if (this.associatedShips[ship_id].orientation == 0) {
          // si le bateau rentre
          if (countOnX + this.associatedShips[ship_id].length <= 10) { //10 = taille de la grille
            // check que de l'eau
            let queDeLeau = true;
            for (let i = 0; i < this.associatedShips[ship_id].length; i++) {
            ...
            if (queDeLeau == true) {
              ...
              if (gameId && numPlayer) {
                this.service.putShip(parseInt(gameId), parseInt(numPlayer), ship_id, countOnY, countOnX, 0)
                  .subscribe(r => {
                    if (r != null)
                      console.log(r);
                  });
        ...
        // verticale
          ...
          // si le bateau rentre
          if (countOnY + this.associatedShips[ship_id].length <= 10) { //10 = taille de la grille
            // check que de l'eau
            let queDeLeau = true;
            for (let i = 0; i < this.associatedShips[ship_id].length; i++) {
            ...
            if (queDeLeau == true) {
              ...
              const numPlayer = this.sessionStorage.getItem('numPlayer');
              if (gameId && numPlayer) {
                this.service.putShip(parseInt(gameId), parseInt(numPlayer), ship_id, countOnY, countOnX, 1)
                  .subscribe(r => {
                    if (r != null)
                      console.log(r);
                  });
