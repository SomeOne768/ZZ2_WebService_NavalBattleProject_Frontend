...
export class PagePregameService {
  ...
  public getAssociatedShip(): Observable<Ship[]> {
    const url = `${environment.apiUrlGame}/Games/AssociatedShip`;
  ...
  public putShip(idGame: number, numPlayer: number, numShip: number, line: number, column: number, orientation: number) {
    const url = `${environment.apiUrlGame}/Games/${idGame}/PutShip/${numPlayer}`;
    const formData = new FormData();
    formData.append('line', line.toString());
    formData.append('numShip', numShip.toString());
